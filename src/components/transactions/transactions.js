...
const Transactions = ({
  ...
  const updateBalance = async () => {
    try {
      ...
      setSuccess(t('Se ha actualizado el saldo correctamente'))
      setTimeout(() => {
        setSuccess('')
        setCustomerId(null)
                            ...
                            <table className="min-w-full" ref={tableRef}>
                              <thead>
                                <tr>
                                  ...
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('paymentId')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Transaction')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('CustomerId')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Username')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Status')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Amount')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Date')}</th>
                                  <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-white-normal tracking-wider text-8x1">{t('Action')}</th>
                                                ...
                                                <span className="relative text-sm text-green-success font-bold">
                                                  {
                                                    item.status === 'NEW' ? (
                                                      <>
                                                        <p className="py-4 whitespace-no-wrap border-b  border-gray-border text-sm leading-5 text-ellipsis uppercase text-yellow-pending font-bold">{t('Pending')}</p>
                                                      </>
                                                    ) :
                                                      item.status === 'COMPLETED' ? (
                                                        <>
                                                          <p className="py-4 whitespace-no-wrap border-b  border-gray-border text-sm leading-5 text-ellipsis uppercase text-green-success font-bold">{t('Activated')}</p>
                                                        </>
                                                      ) :
                                                        item.status === 'EXPIRED' ? (
                                                          <>
                                                            <p className="py-4 whitespace-no-wrap border-b  border-gray-border text-sm leading-5 text-ellipsis uppercase text-gray-info font-bold">{t('Expired')}</p>
                                                          </>
                                                        ) : null
                                                  }
                                                </span>
                                ...
                                <div className="relative p-4 w-full max-w-md h-full md:h-auto">
                                  ...
                                  <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <form className="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8" action="#">
                                      <h3 className="text-xl font-medium text-gray-900 dark:text-white uppercase">{t('Package_activation')}</h3>
                                      <div>
                                      ...
                                      <div>
                                        <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white-normal">{t('Admin_Name')}</label>
                                      ...
                                      <div>
                                        <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white-normal">{t('Customer_Id')}</label>
                                      ...
                                      <div>
                                        <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white-normal">{t('Transaction_Code')}</label>
                                      ...
                                      <div>
                                        <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white-normal">{t('Transaction_Currency')}</label>
                                      ...
                                      <div>
                                        <label htmlFor="password" className="block mb-2 text-sm text-gray-900 dark:text-white-normal font-semibold">{t('Transaction_Amount')}</label>
                                        ...
                                        <td className="border p-4 dark:border-black-background">
                                          {
                                            item.length < 0 ? (
                                              <>
                                                <tr className='text-8x1'>{t('Heres_where_youre_going_to_see_all_your_transactions')}</tr>
                                              </>
                                              ...
                                              <>
                                                {
                                                  item.status === 'COMPLETED' ?
                                                    (
                                                      <div className="text-sm leading-5 0 text-green-success uppercase font-semibold">{t('Completed')}</div>

                                                    ) : item?.status === 'PENDING' ? (
                                                      <div className="text-sm leading-5 text-badges-gold uppercase font-semibold">{t('Pending')}</div>
                                                    ) : item.status === 'NEW' && (
                                                      <div className="text-sm leading-5 text-badges-gold uppercase font-semibold">{t('Open')}</div>
                                                    )
                                                }
                                              </>
                                            )
                                ...
                                <tbody className='flex justify-between items-center w-full' >
                                  <tr className="text-gray-700 flex justify-between items-center w-full">
                                    <td className='text-center my-10 w-full flex flex-col justify-center items-center'>
                                      <p className='text-3xl'>{t('Heres_where_youre_going_to_see_all_your_transactions')}</p>
