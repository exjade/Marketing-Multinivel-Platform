        ...
        _data.push(
            await db.collection('users').get().then(snapshot => {
                snapshot.forEach(doc => {
                    ...
                    const limitContract = applied >= 1 && applied < 100 && parseInt(profit) < 200 / 100 * parseInt(applied)
                    ...
                    if (limitContract) {
                        var sfRef = db.collection('users')
                            .doc(doc.id);
                        batch.update(sfRef, {
                            Balance: user?.Balance + 1.8 / 100 * parseInt(applied),
                ...
                snapshot.forEach(doc => {
                    ...
                    if (limitContract) {
                        var sfRef = db.collection('users')
                            .doc(doc.id);
                        batch.update(sfRef, {
                            Profit: user.Profit + 1.8 / 100 * parseInt(applied),
                        ...
                        //         lastProfitUpdate: admin.firestore.FieldValue.serverTimestamp()
                        //     });
                        return batch.commit();
                    } else {
...
exports.DailyBalanceMondayToFridayInitial = functions.pubsub
    .schedule('0 1 * * 1-5') // MONDAY TO FRIDAY
    .timeZone('America/Mexico_City')
    .onRun(async () => {
        const _data = []
        _data.push(
...
exports.DailyProfitMondayToFridayInitial = functions.pubsub
    .schedule('20 1 * * 1-5') // MONDAY TO FRIDAY
    .timeZone('America/Mexico_City')
    .onRun(async () => {
        const _data = []
        _data.push(
